...
public class GetHash {
    ...
    public void hashLesen(File dir1, FileDataList fileDataList, int anzThread, boolean rekursiv) {
        this.anzThread = anzThread;
        this.rekursiv = rekursiv;
        max = 0;
        progress = 0;
        stop = false;
        HashErstellen hashErstellen = new HashErstellen(dir1, fileDataList);
        runThreads = 1;
    ...
    public void hashSchreiben(FileDataList fileDataList, File fileHash) {
        ...
        progress = 0;
        stop = false;
        new HashFileRead(fileHash, fileDataList);
        runThreads = 1;
        HashTools.schreiben(fileHash, fileDataList);
    }
    ...
    private class HashFileRead implements Runnable {
        ...
        private File hashOutFile;
        private FileDataList fileDataList;
        private LinkedList<File> listeFile = new LinkedList<>();

        public HashFileRead(File hashOutFile, FileDataList fileDataList) {
            this.hashOutFile = hashOutFile;
            this.fileDataList = fileDataList;
        }
        ...
        private void laden() {
            ...
            try {
                in = new LineNumberReader(new InputStreamReader(new FileInputStream(hashOutFile)));
                String zeile;
                while (!stop && (zeile = in.readLine()) != null) {
                    try {
                        fileDataList.add(HashTools.getFileData(zeile));
                    } catch (Exception ex) {
